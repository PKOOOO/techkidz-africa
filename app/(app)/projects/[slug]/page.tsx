...
export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const heroImageUrl = project.heroImage ? urlFor(project.heroImage).url() : null;

    // Process sections to include image URLs
    const sectionsWithImages = project.sections?.map((section) => ({
        ...section,
            ...
            <section className="section-padding">
                <div className="container-custom">
                    <div className="mb-8 text-center max-w-2xl mx-auto">
                        <Link href="/projects" className="text-sm text-gray-500 hover:text-swahilipot-600">
                            ← Back to Projects
                        </Link>
                    </div>
                    ...
                    {sectionsWithImages.length > 0 ? (
                        ...
                        <p className="text-center text-gray-500">No content sections yet. Add them in Sanity Studio.</p>
                    )}
                </div>
            </section>
        </>
